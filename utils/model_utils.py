...
def replicate_model(model, suffix):
    ...
    for weight in model.weights:
        weight._handle_name = "{}/{}".format(suffix, weight.name)  # pylint: disable=protected-access
    return model
...
def specify_regularizers(model,
                         kernel_regularization_factor=0,
                         bias_regularization_factor=0,
                         gamma_regularization_factor=0,
                         beta_regularization_factor=0):

    def _assign_regularizer(layer, attribute_name, switch_name,
                            regularization_factor, summary):
...
def specify_trainable(model, trainable, keywords=None):
    ...
    def _specify_trainable(item, trainable):
        if hasattr(item, "trainable") and item.trainable != trainable:
            print("Change the trainable property of {} to {} ...".format(
                item.name, trainable))
            item.trainable = trainable

    match_function = lambda item: np.sum(
        [item.name.startswith(keyword) for keyword in keywords]) > 0
