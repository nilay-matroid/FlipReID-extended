...
class GlobalGeMPooling2D(GlobalPooling2D):
    ...
    def __init__(self, initial_power=1.0, trainable_power=True, **kwargs):
        ...
        self.reduction_indices = tf.constant([1, 2] if self.data_format ==
                                             "channels_last" else [2, 3])
    ...
    def call(self, inputs):
        return gem_pooling(feature_map=inputs,
                           axis=self.reduction_indices,
                           power=self.power)
