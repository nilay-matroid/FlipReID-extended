...
"""
    ...
    Base: https://github.com/osmr/imgclsmob/blob/v0.0.540/tensorflow2/tf2cv/models/resnesta.py
"""

__all__ = [
    'ResNeStA', 'resnestabc14', 'resnesta18', 'resnestabc26', 'resnesta50',
    'resnesta101', 'resnesta152', 'resnesta200', 'resnesta269',
    'ResNeStADownBlock'
]
...
from applications.common import conv1x1_block, conv3x3_block, saconv3x3_block, AvgPool2d, SimpleSequential, is_channels_first
from applications.senet import SEInitBlock
...
class ResNeStAUnit(nn.Layer):
    ...
    def __init__(self,
        ...
        if bottleneck:
            self.body = ResNeStABottleneck(in_channels=in_channels,
                                           out_channels=out_channels,
                                           strides=strides,
                                           data_format=data_format,
                                           name="body")
        else:
            self.body = ResNeStABlock(in_channels=in_channels,
                                      out_channels=out_channels,
                                      strides=strides,
                                      data_format=data_format,
                                      name="body")
        if self.resize_identity:
            self.identity_block = ResNeStADownBlock(in_channels=in_channels,
                                                    out_channels=out_channels,
                                                    strides=strides,
                                                    data_format=data_format,
                                                    name="identity_block")
    ...
    def __init__(self,
        ...
        for i, channels_per_stage in enumerate(channels):
            ...
            for j, out_channels in enumerate(channels_per_stage):
                strides = 2 if (j == 0) and (i != 0) and (
                    i != len(channels) - 1) else 1
                stage.add(
                    ResNeStAUnit(in_channels=in_channels,
                                 out_channels=out_channels,
                                 strides=strides,
                                 bottleneck=bottleneck,
                                 data_format=data_format,
                                 name="unit_{}_{}".format(i, j + 1)))
                in_channels = out_channels
            self.features.add(stage)
        self.features.add(
            nn.GlobalAvgPool2D(data_format=data_format, name="final_pool"))

        self.output1 = SimpleSequential(name="output1")
        if dropout_rate > 0.0:
            self.output1.add(
                nn.Dropout(rate=dropout_rate, name="output1/dropout"))
        self.output1.add(
            nn.Dense(units=classes, input_dim=in_channels, name="output1/fc"))
...
def get_resnesta(blocks,
    ...
    if width_scale != 1.0:
        channels = [[
            int(cij * width_scale) if (i != len(channels) - 1) or
            (j != len(ci) - 1) else cij for j, cij in enumerate(ci)
        ] for i, ci in enumerate(channels)]
        init_block_channels = int(init_block_channels * width_scale)

    net = ResNeStA(channels=channels,
    ...
    if pretrained:
        ...
        from tf2cv.models.model_store import get_model_file
        in_channels = kwargs["in_channels"] if ("in_channels" in kwargs) else 3
        input_shape = (1,) + (in_channels,) + net.in_size if net.data_format == "channels_first" else\
            (1,) + net.in_size + (in_channels,)
        net.build(input_shape=input_shape)
        net.load_weights(filepath=get_model_file(
...
def resnesta200(in_size=(256, 256), **kwargs):
    ...
    root : str, default '~/.tensorflow/models'
        Location for keeping the model parameters.
    """
    return get_resnesta(blocks=200,
                        in_size=in_size,
                        dropout_rate=0.2,
...
def resnesta269(in_size=(320, 320), **kwargs):
    ...
    root : str, default '~/.tensorflow/models'
        Location for keeping the model parameters.
    """
    return get_resnesta(blocks=269,
                        in_size=in_size,
                        dropout_rate=0.2,
