...
def apply_random_erasing(image_content,
                         sl=0.02,
                         sh=0.4,
                         r1=0.3,
    ...
    for _ in range(max_attempt_num):
        ...
        if erasing_width < image_width and erasing_height < image_height:
            ...
            image_content[starting_height:starting_height + erasing_height,
                          starting_width:starting_width + erasing_width] = mean
            break
    return image_content


def apply_random_grayscale_patch_replacement(image_content,
                                             sl=0.02,
                                             sh=0.4,
                                             r1=0.3,
                                             max_attempt_num=100):
    ...
    for _ in range(max_attempt_num):
        ...
        if erasing_width < image_width and erasing_height < image_height:
            starting_height = np.random.randint(0,
                                                image_height - erasing_height)
            starting_width = np.random.randint(0, image_width - erasing_width)
            patch_in_RGB = image_content[starting_height:starting_height +
                                         erasing_height,
...
class RandomGrayscalePatchReplacement(ImageOnlyTransform):  # pylint: disable=abstract-method
